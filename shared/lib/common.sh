...
# silently running against a placeholder project is worse than failing.
load_env() {
  local dir="${1:-.}"
  if [ -f "$dir/.env" ]; then
    set -a; . "$dir/.env"; set +a
...
# decides — because someone may be running the hosted portal or the Marketplace app.
...
# require_kineviz_desktop <floor-version>
#
# Three things only the person can do: create the account, install, sign in.
# We check what we can, then stop with all three spelled out. See AGENTS.md.
require_kineviz_desktop() {
  local floor="$1"
  ...
  die "Kineviz Desktop not found." "$(cat <<EOF
three steps, all yours — an agent cannot do any of them for you.
      1. Create a Kineviz account (free for individual use, forever): https://www.kineviz.com/
