# Service account keys for Kineviz + BigQuery
...
|---|---|---|
| [`roles/bigquery.dataViewer`](https://cloud.google.com/bigquery/docs/access-control#bigquery.dataViewer) | Read table data and metadata | Reading the graph |
| [`roles/bigquery.jobUser`](https://cloud.google.com/bigquery/docs/access-control#bigquery.jobUser) | Run jobs in the project | Running queries, and listing datasets |

Those two are the whole set for production use.

**Do not grant `roles/bigquery.admin` or `roles/editor`.** They're broad, they're what a
...
`roles/bigquery.dataEditor` **for that work only**, and use a separate account from the one
