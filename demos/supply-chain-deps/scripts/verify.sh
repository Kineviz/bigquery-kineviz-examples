#!/usr/bin/env bash
# Prove the graph works with a real GQL query.
#
# This asserts; it does not describe. Reporting success without running this is
# the one failure mode a demo repo cannot recover from.
set -euo pipefail
...
out=$(bq --project_id="$GCP_PROJECT" query \
        ...
        "GRAPH \`$GCP_PROJECT.$BQ_DATASET.$BQ_GRAPH\`
         MATCH (d:Package)-[:DEPENDS_ON]->(p:Package)
         RETURN p.name AS package, COUNT(DISTINCT d.id) AS dependents
  ...
  case "$e" in
    ...
    *)
      die "Verification query failed: ${e:-unknown error}" \
          "Re-run './gxr up supply-chain-deps' — setup is idempotent. If it persists, open an issue with this output." ;;
  esac
}

rows=$(printf '%s\n' "$out" | tail -n +2 | grep -c . || true)
[ "${rows:-0}" -ge 1 ] || die "Graph query returned no rows." \
  "The graph exists but is empty. Re-run setup; if it persists, try a different DEPS_SYSTEM in .env."
